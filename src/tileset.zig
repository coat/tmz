...
pub const Tileset = struct {
    ...
    pub fn jsonParseFromValue(allocator: Allocator, source: Value, options: ParseOptions) !@This() {
        const tileset = try tmz.jsonParser(@This(), allocator, source, options);
        ...
        if (tileset.source) |s| {
            ...
            const json_value = std.json.parseFromSliceLeaky(Value, allocator, tileset_bytes, .{}) catch @panic("could not parse tileset json");
            return std.json.parseFromValueLeaky(Tileset, allocator, json_value, .{ .ignore_unknown_fields = true }) catch @panic("could not parse tileset");
        }
        return tileset;
    }
};
...
pub fn loadTilesetFromFile(allocator: Allocator, file_path: []const u8) !ManagedTileset {
    const file = try std.fs.cwd().openFile(file_path, .{});
    const project_json = try file.reader().readAllAlloc(allocator, std.math.maxInt(u32));
    defer allocator.free(project_json);

    return try loadTileset(allocator, project_json);
}

test "loadTilesetFromFile works" {
    const managed_tileset = try loadTilesetFromFile(std.testing.allocator, "src/test/tileset.tsj");
...
pub fn loadTileset(allocator: Allocator, tileset_json: []const u8) !ManagedTileset {
...
test "loadTileset works" {
    const test_tileset_file = @embedFile("test/tileset.tsj");
    const managed_tileset = try loadTileset(std.testing.allocator, test_tileset_file[0..]);
    defer managed_tileset.deinit();
...
fn testTileset(tileset: Tileset) !void {
    ...
    try expectEqualStrings("../../../stxy/src/tiles.png", tileset.image.?);
    try expectEqual(448, tileset.image_height.?);
    try expectEqual(592, tileset.image_width.?);
    try expectEqual(0, tileset.margin.?);
    try expectEqualStrings("tiles", tileset.name.?);
    try expectEqual(0, tileset.spacing.?);
    try expectEqual(1036, tileset.tile_count.?);
    ...
    try expectEqual(62, wang_tiles.len);
