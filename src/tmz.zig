...
// ignores data, value andr properties so they can be handled later
pub fn jsonParser(T: type, allocator: Allocator, source: Value, options: ParseOptions) !T {
    var t: T = undefined;

    inline for (@typeInfo(T).Struct.fields) |field| {
        if (comptime std.mem.eql(u8, field.name, "data") or std.mem.eql(u8, field.name, "value") or std.mem.eql(u8, field.name, "properties")) continue;
